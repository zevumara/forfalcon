...
class installModel extends Model
{
	...
	public function install($prefix, $username, $password, $name, $email, $language)
	{	
		$this->db->query("
		...
		CREATE TABLE " . $prefix . "users(
			...
			rank ENUM('administrator', 'grade3', 'grade2', 'grade1', 'moderator', 'user'),
		...
		CREATE TABLE " . $prefix . "modules(
			id INT(4) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
			name VARCHAR(50) NOT NULL,
			access ENUM('administrator', 'grade3', 'grade2', 'grade1', 'moderator', 'user'),
			position INT(4) UNSIGNED
		);
	...
	public function isInstalled()
	{
		try
		{
			$administrator = $this->db->query("SELECT id FROM " . DB_PREFIX . "users WHERE rank = 'administrator' LIMIT 1");
